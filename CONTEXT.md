...
**"Additive" describes the two applications that admit a newcomer, not all three.** Sign in with LinkedIn now appears on the attendee app, the meetings portal and the sponsor portal, reading one shared module (`packages/db/src/linkedin-identity.ts`, deep-imported). On the first two it may create an account, and the gate then meets that person with the checklist. On the **sponsor portal it can only ever join an account that already exists**: that portal admits sponsor representatives and WBR-side roles only, the role a new account would be given is neither, and the decision refuses the create before any row is written. Someone with no account there is sent back to the sign-in screen with the cause named, rather than being given a row and then turned away. The role is asked on both paths — the person who exists and the person who does not — and the role a create is tested against is the role it writes.
