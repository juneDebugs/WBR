#!/usr/bin/env node
// Browser E2E for the meetings dashboard "Your Team at WBR 2027" module (the
// replica of the sponsor app's module):
//   Goal 1 — the section renders on / (user dashboard) with one card per WBR
...
//
//   SMOKE_BASE_URL=http://localhost:3002 node scripts/e2e-meetings-team.mjs           # server already up
//   SMOKE_BASE_URL=http://localhost:3002 node scripts/e2e-meetings-team.mjs --start   # boot next dev, then kill
//
// Env: SMOKE_BASE_URL, SMOKE_EMAIL, SMOKE_PASSWORD (attendee), SHOT_DIR.
...
const CREDS = { email: process.env.SMOKE_EMAIL ?? 'stephcurry@test.com', password: process.env.SMOKE_PASSWORD ?? 'password123' }
...
async function main() {
  ...
  check('API exposes a staff array', Array.isArray(staff), 'is /api/dashboard returning `staff`?')
  ...
  if (expectedCount === 0) {
    ...
    check('section heading visible on the dashboard', shown)
    ...
    await page.screenshot({ path: join(SHOT_DIR, 'meetings-team-section.png'), fullPage: true }).catch(() => {})
    ...
    if (dialogShown) {
      ...
      await page.screenshot({ path: join(SHOT_DIR, 'meetings-team-dialog.png') }).catch(() => {})
