#!/usr/bin/env node
// Applies the MeetingTableSetting DDL to the runtime database (Turso).
// Idempotent.
//
...
// meeting-tables (admin-managed physical table inventory) feature. The DDL
// below matches the MeetingTableSetting model in
...
//
// Usage: node scripts/migrate-meeting-tables.mjs [--local <path/to/dev.db>]
...
const DDL = [
  `CREATE TABLE IF NOT EXISTS "MeetingTableSetting" (
...
async function main() {
  ...
  console.log(`Applying MeetingTableSetting DDL to ${target.replace(/\/\/.*@/, '//***@')}`)
  for (const stmt of DDL) {
    await client.execute(stmt)
  }
  const check = await client.execute(
    `SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'MeetingTableSetting'`
  )
  if (check.rows.length !== 1) {
    console.error('✗ MeetingTableSetting table missing after migration')
    process.exit(1)
  }
  console.log('✓ MeetingTableSetting table present')
  client.close?.()
}
