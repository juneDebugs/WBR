#!/usr/bin/env node
// Applies the MeetingRequirementSetting DDL to the runtime database (Turso).
// Idempotent.
//
...
// meeting-requirements (admin-configurable attendee/sponsor meeting targets)
// feature. The DDL below matches the MeetingRequirementSetting model in
// packages/db/prisma/schema.prisma exactly, and the same DDL is created
// defensively at runtime by packages/db/src/meeting-engine.ts — this script
// just guarantees it up front.
//
// Usage: node scripts/migrate-meeting-requirements.mjs [--local <path/to/dev.db>]
...
const DDL = [
  `CREATE TABLE IF NOT EXISTS "MeetingRequirementSetting" (
    "scope" TEXT NOT NULL,
...
async function main() {
  ...
  console.log(`Applying MeetingRequirementSetting DDL to ${target.replace(/\/\/.*@/, '//***@')}`)
  for (const stmt of DDL) {
    await client.execute(stmt)
  }
  const check = await client.execute(
    `SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'MeetingRequirementSetting'`
  )
  if (check.rows.length !== 1) {
    console.error('✗ MeetingRequirementSetting table missing after migration')
    process.exit(1)
  }
  console.log('✓ MeetingRequirementSetting table present')
  client.close?.()
}
