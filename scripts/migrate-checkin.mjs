#!/usr/bin/env node
// Idempotent migration for the on-site floor check-in portal.
// Adds SponsorMeeting.sponsorArrivedAt / SponsorMeeting.buyerArrivedAt
// (DATETIME, nullable) so floor managers can tick off arrivals per meeting.
//
...
//
//   node scripts/migrate-checkin.mjs            # auto-detect Turso, else local
//   node scripts/migrate-checkin.mjs --local    # force local dev.db
//
...
async function main() {
  ...
  for (const col of ['sponsorArrivedAt', 'buyerArrivedAt']) {
    if (cols.has(col)) {
      console.log(`  ✓ SponsorMeeting.${col} already present`)
    } else {
      await db.execute(`ALTER TABLE "SponsorMeeting" ADD COLUMN "${col}" DATETIME`)
      changed++
      console.log(`  + added SponsorMeeting.${col} (DATETIME NULL)`)
    }
  }
