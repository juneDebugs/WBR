#!/usr/bin/env node
// Idempotent migration for the mutual Best Fit auto-match log.
// Creates the AutoMatchEvent table (relation-free, name-denormalized audit
// trail behind the admin Meetings → Auto tab) plus its two indexes, matching
// the AutoMatchEvent model in packages/db/prisma/schema.prisma exactly.
//
...
//
//   node scripts/migrate-auto-match.mjs            # auto-detect Turso, else local
//   node scripts/migrate-auto-match.mjs --local    # force local dev.db
//
...
async function main() {
  ...
  const tables = await db.execute(`SELECT name FROM sqlite_master WHERE type='table' AND name='AutoMatchEvent'`)
  if (tables.rows.length > 0) {
    console.log('  ✓ AutoMatchEvent table already present')
  } else {
    await db.execute(`CREATE TABLE "AutoMatchEvent" (
    ...
    changed++
    console.log('  + created AutoMatchEvent table')
  }

  const idx = await db.execute(`PRAGMA index_list("AutoMatchEvent")`)
  const have = new Set(idx.rows.map(r => String(r.name)))
  for (const [name, cols] of [
    ['AutoMatchEvent_sponsorId_userId_idx', '"sponsorId", "userId"'],
    ['AutoMatchEvent_createdAt_idx', '"createdAt"'],
  ]) {
    if (have.has(name)) {
      console.log(`  ✓ ${name} already present`)
    } else {
      await db.execute(`CREATE INDEX "${name}" ON "AutoMatchEvent"(${cols})`)
      changed++
