...
- **Narrowed for seeded data on 2026-08-01, at the start of the Phase 8 implementation.** The venue-map record's image column holds a **string URL**, matching the existing `Sponsor.logoUrl` and `Conference.logoUrl` contract. The seeded demonstration maps are committed PNG files and the column holds their path; the base64 pattern above still governs the Phase 10 upload path, which writes a data URL into the same column with no change to the viewer. The measurements behind this — no floor-plan picture exists in the repository, no seeded picture in the repository is base64, and inline base64 images distort this project's performance measurements — are recorded as finding F-8 in the engineer-local floor-plan PRD. This changes where a seeded picture lives, not the decision this ADR makes about pins over a raster image.
- **Narrowed for organizer uploads on 2026-08-02, at the start of the Phase 10 implementation: the upload accepts JPG and PNG only, and refuses a PDF.** The refusal message tells the organizer to save the file as an image and upload that. This is recorded here, and not only in the engineer-local PRD, because it works against one of the consequences this ADR claims above — that whatever picture arrives, including a PDF, the feature works. The trade was taken knowingly by the project owner: the person handing the file over is the conference organizer, who already holds it and has a way to convert it, not the venue replying with whatever it happens to have. Measured at the time: no PDF library was ever installed in this repository, so this withdraws an unbuilt requirement rather than removing working behaviour. The reasoning, the withdrawn user story, and the demonstration risk if a PDF arrives on the day are finding F-15 in the engineer-local floor-plan PRD. This changes which file formats the upload accepts; it does not change the decision this ADR makes about pins over a raster image.
- **The uploaded picture is served from its own address, decided 2026-08-02 at the start of Phase 10.** Storage is unchanged and still follows ADR 0004: the image column holds a base64 data URL. Delivery changed. `GET /api/data/map` returns every map for the conference in one response, so an uploaded picture in that column would be sent in full, for every map, on every visit to the map screen, even though a delegate views one map at a time. Measured: the three seeded pictures total 98,614 bytes on disk and roughly 131,000 as base64 text, and that is the floor rather than the ceiling once real venue plans replace line drawings. The response now carries a short string for every map — a file path for a seeded one, `/api/data/map/<id>/image` for an uploaded one — and that address decodes and serves the bytes. This follows the existing precedent for speaker photographs in the admin app, which substitutes a dedicated address for any stored value beginning `data:`. Recorded as finding F-14 in the engineer-local floor-plan PRD. It reinforces rather than revises the image-weight consequence recorded above.
