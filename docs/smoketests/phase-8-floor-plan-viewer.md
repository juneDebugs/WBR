...
|---|---|---|---|---|
| 1 | The marker layer is padded so it stops being the picture's box | 6 | 6 | caught as predicted |
| 2 | The completeness guard is removed from `GET /api/data/map` | 1 | 1 | caught as predicted |
| 3 | The tap target shrinks from 44 to 24 pixels, marker centre unchanged | 1 | 1 | caught as predicted |
| 4 | Room labels stay in the markup but are hidden from the screen | 4 | 4 | caught as predicted |
...
| Negative controls | contract | C | 4 of 4 caught as predicted |

Steps 2 to 10 are the single Playwright run: **54 of 54**. It was 48 before the adversarial review added the real-pixel containment and separation assertions and the per-map label-bounds assertions.
