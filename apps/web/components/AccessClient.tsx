  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {STAT_CARDS.map(card => (
          <Link
            key={card.key}
            href={card.href}
            className="group relative block bg-white border border-hairline rounded-xl p-4 cursor-pointer transition-all duration-150 hover:bg-fill hover:border-hairline hover:shadow-sm active:scale-[0.99] active:bg-fill focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:ring-offset-2"
            data-stat={card.key}
            data-value={counts[card.key]}
            aria-label={`${card.label}: ${counts[card.key].toLocaleString()} — ${card.hint}`}
          >
            <svg
              className="absolute top-4 right-4 w-4 h-4 text-ink-3 transition-colors duration-150 group-hover:text-primary"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2}
            ...
            <p className="text-title1 leading-9 font-semibold tracking-tight text-ink tabular-nums">
              {counts[card.key].toLocaleString()}
            </p>
            <p className="text-footnote text-ink-2 font-medium">{card.label}</p>
            <p className="text-caption text-ink-2 mt-0.5">{card.hint}</p>
          </Link>
        ))}
      </div>
      ...
      <div className="bg-white border border-hairline rounded-xl overflow-hidden">
        <div className="flex border-b border-hairline" role="tablist" aria-label="User list filters">
        ...
        {tab === 'invite' ? (
          <div className="p-6 max-w-md">
            <h3 className="font-semibold text-ink mb-1">Add or update a user</h3>
            <p className="text-sm text-ink-2 mb-4">
              If the email already exists their role will be updated. Otherwise a new account is created.
            </p>
            <form onSubmit={handleInvite} className="space-y-3">
              <div>
                <label htmlFor="invite-email" className="block text-xs font-medium text-ink-2 mb-1">Email *</label>
                <input
                  id="invite-email"
                  type="email"
                  required
              ...
              <div>
                <label htmlFor="invite-name" className="block text-xs font-medium text-ink-2 mb-1">Display name</label>
                <input
                  id="invite-name"
                  type="text"
                  value={inviteName}
              ...
              <div>
                <label htmlFor="invite-role" className="block text-xs font-medium text-ink-2 mb-1">Role</label>
                <select
                  id="invite-role"
                  value={inviteRole}
                  onChange={e => setInviteRole(e.target.value)}
                  className="w-full px-3 py-2 text-sm border border-hairline rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/30 bg-white"
                >
              ...
              {inviteStatus === 'err' && <p className="text-sm text-danger-ink" role="alert">{inviteErr}</p>}
              {inviteStatus === 'ok' && <p className="text-sm text-success-ink" role="status">User saved successfully.</p>}
              <button
                type="submit"
          ...
          <>
            <div className="divide-y divide-hairline" aria-busy={isLoading}>
              {rows.length === 0 && (
              ...
              {rows.map(user => (
                <div key={user.id} className="flex items-center gap-3 px-4 py-3 hover:bg-fill">
                  {user.image ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={closeCred}>
          ...
          >
            <div className="flex items-start justify-between mb-5">
              <div>
                <h2 className="font-semibold text-ink text-lg">User Credentials</h2>
