...
- **App-to-app API call (cross-app, not cross-process).** Writers in this app call `lib/revalidate-attendee.ts`, which posts `{ secret, tags }` to the attendee app's `/api/revalidate` so that app's `unstable_cache` rebuilds. Speaker edits send `speakers`; map, marker and sponsor-detail edits send `floor-plan`. Documented as the system's only app-to-app traffic in [`docs/architecture.md`](../../docs/architecture.md) §System diagram (dotted arrow).

  **Corrected 2026-08-02.** This entry previously said the URL was "hardcoded to localhost" and that in production "the request fails and the `catch` block silently no-ops". The first half stopped being true when the helper was changed to read `ATTENDEE_APP_URL`. The second half was true in a worse way than it described, and is now fixed — see below. A reader trusting the old wording would conclude the mechanism cannot work and build a second one.

  **Three things this depends on, each of which was broken and is now fixed:**

  1. **`ATTENDEE_APP_URL` must be set** in this app's deployed environment, or the request goes to localhost. Measured 2026-08-02: it was set on no Vercel project and named in no example file. Now documented in `.env.local.example`.
  2. **`NEXTAUTH_SECRET` must be identical** in this app and the attendee app — it is what the receiving route checks. Measured 2026-08-02: the four apps on one development machine held three different values, so this had never worked locally for any tag.
  3. **The attendee app's middleware must let the request through.** It guards everything its matcher covers and answers 401 to anything without a session cookie, which a server-to-server call does not have — so the receiving route never ran, for any tag, in any environment. `apps/attendee/middleware.ts` now exempts exactly `/api/revalidate` by path equality, leaving the shared-secret check as the authentication.

  **Why none of this was noticed:** `fetch` does not throw on an HTTP error status. The old helper had only a `.catch()` and never inspected `res.ok`, so a 401 produced no log line at all and every call appeared to succeed. The helper now warns on any non-OK response, naming the status, the address and the tags that were not cleared. Full analysis in the floor-plan PRD at findings F-16 and F-17.
