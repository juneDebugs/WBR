...
const getCachedDashboardStats = unstable_cache(
  async () => {
    ...
    for (const { results } of withScores) {
      for (const r of results) { if (!r.done) missingCounts[r.label] = (missingCounts[r.label] ?? 0) + 1 }
    }
    ...
    return {
      ...
      conference: conference
        ? { id: conference.id, name: conference.name, venue: conference.venue, startDate: conference.startDate.toISOString(), endDate: conference.endDate.toISOString() }
        : null,
      sponsorReadiness: {
        sponsors: withScores,
