...
import { useSponsorData } from '@/lib/hooks'
    ...
    <header className="bg-white border-b border-gray-100 sticky top-0 z-40 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-14">
        ...
        <div className="flex items-center gap-3">
          {logoUrl ? (
            <img src={logoUrl} alt={sponsorName ?? 'Sponsor'} width={28} height={28} className="w-7 h-7 rounded-lg object-contain bg-white border border-gray-100" />
          ) : (
          ...
          {NAV.map(({ href, label, icon }) => (
            <Link key={href} href={href}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                active(href) ? 'bg-primary/10 text-primary' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800'
              }`}>
        ...
        <div className="flex items-center gap-2">
          {role === 'STAFF' && (
            <span className="badge bg-violet-100 text-violet-700 hidden sm:flex">Staff</span>
          )}
