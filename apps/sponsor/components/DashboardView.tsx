...
export function DashboardView() {
  ...
  const statCards = [
    { label: 'Total Requests', value: stats?.totalMeetings ?? 0, color: 'text-primary', bg: 'bg-primary/10', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z' },
    { label: 'Pending', value: stats?.pendingCount ?? 0, color: 'text-amber-600', bg: 'bg-amber-50', icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' },
    { label: 'Confirmed', value: stats?.confirmedCount ?? 0, color: 'text-emerald-600', bg: 'bg-emerald-50', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
  ...
  if (sponsorLoading) {
    return (
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
        <div><div className="h-7 w-64 bg-gray-200 rounded animate-pulse" /><div className="h-4 w-40 bg-gray-100 rounded animate-pulse mt-2" /></div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[1,2,3,4].map(i => (<div key={i} className="card p-5"><div className="w-10 h-10 rounded-xl bg-gray-100 animate-pulse mb-3" /><div className="h-7 w-12 bg-gray-200 rounded animate-pulse" /><div className="h-3 w-20 bg-gray-100 rounded animate-pulse mt-2" /></div>))}
        </div>
      </div>
    )
  }
      ...
      {conflicts.length > 0 && (
        <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 flex items-start gap-3">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-red-800">{conflicts.length} presenter scheduling conflict{conflicts.length !== 1 ? 's' : ''} detected</p>
            <p className="text-xs text-red-600 mt-0.5">{conflicts.map((c: any) => c.speakerName).join(', ')} {conflicts.length === 1 ? 'is' : 'are'} double-booked. Contact the organizer.</p>
          </div>
        </div>
      )}

      <div>
        <h1 className="text-2xl font-bold text-gray-900">{userRole === 'STAFF' ? 'Staff Dashboard' : `Welcome back, ${userName.split(' ')[0]}`}</h1>
        <p className="text-gray-500 text-sm mt-1">{sponsor?.name ?? 'WBR'} · Sponsor Portal</p>
      </div>
        ...
        {sponsor && (
          <div className="card p-6 space-y-4">
            ...
            <div className="w-full bg-gray-100 rounded-full h-2.5">
              <div className="h-2.5 rounded-full transition-all" style={{ width: `${profile.score}%`, background: profile.score >= 80 ? '#10b981' : '#f59e0b' }} />
            </div>
            ...
            {profile.missing.length > 0 && (
              <div>
                ...
                <ul className="space-y-1.5 max-h-32 overflow-y-auto pr-1">
                  {profile.missing.map(f => (<li key={f} className="flex items-center gap-2 text-xs text-amber-700"><span className="w-1.5 h-1.5 rounded-full bg-amber-400 flex-shrink-0" />{f}</li>))}
                </ul>
              </div>
            )}
          </div>
            ...
            <div className="space-y-3">
              {recentRequests.map((r: any) => (
                <div key={r.id} className="flex items-center gap-3">
                  ...
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full flex-shrink-0 ${r.status === 'PENDING' ? 'bg-amber-50 text-amber-700' : r.status === 'CONFIRMED' || r.status === 'APPROVED' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'}`}>{r.status}</span>
                </div>
              ))}
            </div>
          )}
            ...
            {upcomingMeetings.map((m: any) => {
              ...
              return (
                <div key={m.id} className="flex items-center gap-3 rounded-xl border border-emerald-100 bg-emerald-50/40 px-4 py-3">
                  {m.person?.image ? (<img src={m.person.image} alt="" className="w-10 h-10 rounded-full object-cover flex-shrink-0" />) : (<div className="w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center flex-shrink-0"><span className="text-sm font-semibold text-emerald-700">{(m.person?.name ?? '?')[0]}</span></div>)}
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900 truncate">{m.person?.name ?? 'Attendee'}</p>
                    <p className="text-xs text-gray-500 mt-0.5">{starts.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })} · {starts.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })}{m.location ? ` · ${m.location}` : ''}</p>
                  </div>
