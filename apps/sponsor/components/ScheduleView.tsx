  ...
  if (isLoading) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-8">
        <div><div className="h-7 w-48 bg-gray-200 rounded animate-pulse" /><div className="h-4 w-32 bg-gray-100 rounded animate-pulse mt-2" /></div>
        {[1, 2, 3].map(i => (
          <div key={i} className="card p-5 flex items-center gap-4">
            <div className="w-16 space-y-1"><div className="h-3 bg-gray-100 rounded animate-pulse" /><div className="h-3 bg-gray-100 rounded animate-pulse" /></div>
            <div className="w-px h-12 bg-gray-100" />
            <div className="w-10 h-10 rounded-full bg-gray-200 animate-pulse" />
            <div className="flex-1 space-y-1"><div className="h-4 w-28 bg-gray-200 rounded animate-pulse" /><div className="h-3 w-40 bg-gray-100 rounded animate-pulse" /></div>
          </div>
        ))}
      </div>
    )
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">{allItems.length} confirmed meeting{allItems.length !== 1 ? 's' : ''} at WBR</p>
      </div>
        ...
        Array.from(byDate.entries()).map(([dateKey, items]) => {
          ...
          return (
            <div key={dateKey} className="space-y-3">
              <h2 className="font-semibold text-gray-700 text-sm uppercase tracking-wide border-b border-gray-100 pb-2">{slot.date}</h2>
              {items.map(item => {
                const s = formatSlot(item.startsAt, item.endsAt)
                return (
                  <div key={item.id} className="card p-5 flex items-center gap-4">
                    <div className="flex-shrink-0 text-center w-16">
                      <div className="text-xs text-gray-400 leading-tight">{s.time.split('–')[0].trim()}</div>
                      <div className="text-xs text-gray-300">–</div>
                      <div className="text-xs text-gray-400 leading-tight">{s.time.split('–')[1]?.trim()}</div>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-gray-500">{item.person?.jobTitle}{item.person?.company ? ` · ${item.person.company}` : ''}</p>
                      {item.location && (
                    ...
                    <span className={`text-xs flex-shrink-0 px-2 py-1 rounded-full font-medium ${item.source === 'admin' ? 'bg-primary/10 text-primary' : 'bg-emerald-50 text-emerald-700'}`}>
