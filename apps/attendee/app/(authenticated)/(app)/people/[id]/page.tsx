    ...
    <div className="min-h-screen" style={{ background: '#f0ece4' }}>
      ...
      <div className="flex flex-col items-center pt-4 pb-5 px-4">
        ...
        {user.image ? (
          ...
          >
            <span className="text-white font-medium text-[38px] leading-none" style={{ letterSpacing: '-0.02em' }}>
              {initials}
            </span>
          </div>
        )}
        ...
        {/* Name */}
        <h1 className="text-[22px] font-bold text-gray-900 mt-3 text-center leading-tight tracking-tight">
          {name}
        </h1>

        {/* Title & Company */}
        {(user.jobTitle || user.company) && (
          <p className="text-[13px] text-gray-500 mt-0.5 text-center leading-snug">
        ...
        <div className="flex justify-center gap-2 px-6 pb-5">
          ...
          {linkedinUrl && (
            <a
              ...
              className="flex flex-col items-center gap-1 flex-1 max-w-[80px] active:opacity-50 transition-opacity"
            >
              <div className="w-[42px] h-[42px] rounded-full bg-[#0A66C2]/10 flex items-center justify-center">
          ...
          {twitterHandle && (
            <a
              ...
              className="flex flex-col items-center gap-1 flex-1 max-w-[80px] active:opacity-50 transition-opacity"
            >
              <div className="w-[42px] h-[42px] rounded-full bg-gray-900/5 flex items-center justify-center">
                <svg className="w-[16px] h-[16px] text-gray-900" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                </svg>
              </div>
              <span className="text-[10px] font-medium text-gray-900">Twitter</span>
            </a>
          )}
      ...
      <div className="px-4 pb-28 space-y-5">
        ...
        {user.bio && (
          <div>
            <p className="text-[12px] font-medium text-gray-400 uppercase tracking-wide px-3 mb-1.5">About</p>
            <div className="bg-white rounded-[14px] px-4 py-3.5" style={{ boxShadow: '0 0.5px 1px rgba(0,0,0,0.04)' }}>
              <p className="text-[15px] text-gray-900 leading-relaxed">{user.bio}</p>
            </div>
          </div>
        )}
        ...
        {hasDetails && (
          <div>
            <p className="text-[12px] font-medium text-gray-400 uppercase tracking-wide px-3 mb-1.5">Info</p>
            <div className="bg-white rounded-[14px] overflow-hidden" style={{ boxShadow: '0 0.5px 1px rgba(0,0,0,0.04)' }}>
              {user.jobTitle && (
                <div className="px-4 py-3 flex items-start gap-3">
                  <svg className="w-[18px] h-[18px] text-gray-400 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[15px] text-gray-900">{user.jobTitle}</p>
                  </div>
                </div>
              )}

              {user.company && user.jobTitle && (
                <div className="mx-4 border-t border-gray-100/80" />
              )}
              ...
              {user.company && (
                <div className="px-4 py-3 flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] text-gray-400 font-medium">Company</p>
                    <p className="text-[15px] text-gray-900">{user.company}</p>
                  </div>
                </div>
              )}
              ...
              {user.website && (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] text-gray-400 font-medium">Website</p>
                    <p className="text-[15px] text-primary truncate">{user.website.replace(/^https?:\/\//, '')}</p>
                  </div>
                  <svg className="w-[14px] h-[14px] text-gray-300 mt-1 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round"
        ...
        {user.speakerProfile && (
          <div>
            <p className="text-[12px] font-medium text-gray-400 uppercase tracking-wide px-3 mb-1.5">Conference</p>
            <Link
              href={`/speakers/${user.speakerProfile.id}`}
              className="bg-white rounded-[14px] px-4 py-3.5 flex items-center gap-3 active:bg-gray-50 transition-colors"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[15px] text-gray-900 font-medium">Speaker Profile</p>
                <p className="text-[12px] text-gray-400 mt-0.5">View sessions and talks</p>
              </div>
              <svg className="w-[14px] h-[14px] text-gray-300 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
