...
export async function fetchSpeakersData() {
  ...
  const data = speakers.map(s => ({
    ...
    sessions: s.confSessions.map(cs => ({
      ...
      startsAt: cs.startsAt instanceof Date ? cs.startsAt.toISOString() : String(cs.startsAt),
      endsAt: cs.endsAt instanceof Date ? cs.endsAt.toISOString() : String(cs.endsAt),
      room: cs.room,
      track: cs.track,
      type: cs.type,
    })),
