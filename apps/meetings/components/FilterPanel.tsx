                  ...
                  {cat.items.map(s => (
                    <button
                      ...
                      className={`flex items-center gap-2 w-full text-left px-2 py-1.5 rounded-lg text-xs transition-colors ${
                        selected.includes(s) ? 'bg-primary/10 text-primary font-medium' : 'text-ink-2 hover:bg-fill'
                      }`}
                    >
                      <span className={`w-3.5 h-3.5 rounded border flex-shrink-0 flex items-center justify-center ${
                        selected.includes(s) ? 'bg-primary border-primary' : 'border-hairline'
                      }`}>
                        {selected.includes(s) && (
...
export function FilterPanel({ filters, onChange, mode }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Search */}
      <div>
        <div className="flex items-center gap-2 px-3 py-2 border border-hairline rounded-xl bg-surface focus-within:ring-2 focus-within:ring-primary/40">
          ...
          />
          {f.search && (
            <button onClick={() => onChange({ ...f, search: '' })} aria-label="Clear search" className="text-ink-3 hover:text-ink-2">
      ...
      {/* Clear */}
      {hasAny && (
        <button onClick={() => onChange(EMPTY_FILTERS)} className="text-xs text-ink-2 hover:text-danger transition-colors">
          Clear all filters
        </button>
      )}
    </div>
